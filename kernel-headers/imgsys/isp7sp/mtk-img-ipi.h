 ...
 ****************************************************************************
 ****************************************************************************/
#ifndef COMMON_KERNEL_HEADERS_IMGSYS_MTK_IMG_IPI_H_
#define COMMON_KERNEL_HEADERS_IMGSYS_MTK_IMG_IPI_H_

#include <linux/types.h>
#include <linux/time.h>
#include <imgsys/isp7sp/mtk_header_desc.h>
...
#define MTK_V4L2_SKIP_TILE_SUPPORT	1
...
/**
 *  @img_swfrm_info
 *  @brief sw information for eatch sub-frame
 *
 *  @var hw_comb         operting hw in sub-frame
 *  @var sw_ridx         adopted ring index for working buf
 *  @var is_time_shared  the sub-frame is time shared or not
 *  @var is_secFrm       the frame is secure frame or not
 *  @var is_earlycb      the sub-frame is with earlyCB or not
 *  @var is_lastingroup  is last frame in current group or not
 *  @var sw_goft         memory offset about g cmd buffer
 *  @var sw_bwoft        memory offset about bw buffer
 *  @var subfrm_idx    	 the sub-frame index in package
 *  @var g_swbuf         kernel va about current gce buffer
 *  @var bw_swbuf        kernel va about current bw buffer
 *  @var pixel_bw        pixel bw about current sub-frame
 */
...
/**
 *  @swfrm_info_t
 *  @brief sw frm information based on whole request
 *
 *  @var req_sbuf_goft   global offset about swfrminfo/req_sbuf (communicat via daemon/kernel)
 *  @var req_sbuf_kva    kva for current info structure
 *  @var swfrminfo_ridx  Ring index for access correct swfrminfo (referenced by daemon)
 *  @var request_fd      Enqueued request fd in v4l2 kernel framework
 *  @var request_no      request no from mw
 *  @var frame_no        frame no from mw
 *  @var frm_owner       frame owner from mw
 *  @var is_secReq       the request is under secure scenario or not
 *  @var fps             fps information
 *  @var eq_tstamp       timestamp about the moment at imgstream enque
 *  @var handle          kernel request mapping handle
 *  @var req_vaddr       kernel request kva
 *  @var sync_id         sync_id information for cam/img sync
 *  @var total_frmnum    total adopted sub-frame number in the current request
 *  @var is_earlycb      the request is separated by early cb hint or not
 *  @var earlycb_sidx    the sub-frame index we need early cb
 *  @var is_lastfrm      the request is separated by lastfrm (which means all frames in enqued package are done)
 *  @var group_id        gruop hint for selecting gce thread
 *  @var fail_uinfo_idx  failed index in user_info (we need to convert this to real subfidx)
 *  @var fail_isHWhang   current fail is caused by hw hang or not
 *  @var eqtime          Enque timestamp (at imgstream) for a request
 */
struct swfrm_info_t {
...
/**
 *  @cleartoken_info_t
 *  @brief tokens to clear
 *
 *  @var clearnum   number of token to clear
 *  @var token      token for clear
 */
...
}__attribute__ ((__packed__)) srz_config;
...
struct dip_param {
	...
	uint8_t num_srz_cfg;
	srz_config srz_cfg[MAX_SRZ_CONFIGS];
...
#endif  /* COMMON_KERNEL_HEADERS_IMGSYS_MTK_IMG_IPI_H_ */
