...
static int vendor_cmd_reply_info_handler(struct nl_msg *msg, void *arg)
{
    ...
    strncpy(reply_info->reply_buf.ptr, nla_data(tb[NL80211_ATTR_VENDOR_DATA]), reply_info->reply_buf_size - 1);
                  ...
                  size_t buf_len)
{
    ...
    if (os_strncasecmp(cmd, "POWERMODE ", 10) == 0) {
        ...
        handled = 0; /* Handle MIRACAST as private cmd*/
        ...
        struct wpa_pmkid_entry *entry =
            (struct wpa_pmkid_entry*) buf;
        ...
        wpa_printf(MSG_INFO, "bssid:" MACSTR ", sta:" MACSTR
             ", PMKID Add-Remove:%d",
             MAC2STR(entry->bssid), MAC2STR(entry->sta),
             entry->addremove);
        ...
        wpa_printf(MSG_INFO, "Transparent command for driver nl80211, result %d, reply_len=%d, reply=%s handled=%d", result, ret, buf, handled);
    }
    ...
    if (handled == 0) {
        ...
        if (ret < 0) {
            ...
            wpa_printf(MSG_INFO, "%s: ret = %d used = %u total = %u buf = %s",
                    __func__, ret , priv_cmd.used_len, priv_cmd.total_len, priv_cmd.buf);
...
void mtk_nl80211_generic_response_event(struct wpa_driver_nl80211_data *drv,
                u8 *data, size_t data_len)
{
    ...
    while (end - pos >= 2) {
        ...
        case MTK_GRID_EXTERNAL_AUTH:
        {
             ...
             event.external_auth.dot11MultiLinkActivated =
                 info->dot11MultiLinkActivated;
