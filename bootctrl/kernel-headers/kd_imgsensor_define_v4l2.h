...
typedef enum {
  IMGSENSOR_TEE_DEV_SENSOR,
  IMGSENSOR_TEE_DEV_SENINF,
  IMGSENSOR_TEE_DEV_MAX_NUM
} IMGSENSOR_TEE_DEV;

typedef struct ACDK_SENSOR_RESOLUTION_INFO_STRUCT {
...
typedef struct ACDK_SENSOR_INFO_STRUCT {
...
typedef struct {
  MUINT32 GroupIdx;
  MUINT32 ItemIdx;
  MUINT8 ItemNamePtr[50];
  MUINT32 ItemValue;
  MINT32 IsTrueFalse;
  MINT32 IsReadOnly;
  MINT32 IsNeedRestart;
  MUINT32 Min;
  MUINT32 Max;
...
typedef struct ACDK_SENSOR_EXPOSURE_WINDOW_STRUCT {
  MUINT16 GrabStartX;
  MUINT16 GrabStartY;
...
typedef struct ACDK_SENSOR_CONFIG_STRUCT {
  ACDK_SENSOR_IMAGE_MIRROR_ENUM SensorImageMirror;
  MINT32 EnableShutterTansfer;
  MINT32 EnableFlashlightTansfer;
  //ACDK_SENSOR_OPERATION_MODE_ENUM SensorOperationMode;
  MUINT16 ImageTargetWidth;
  MUINT16 ImageTargetHeight;
  MUINT16 CaptureShutter;
  MUINT16 FlashlightDuty;
  MUINT16 FlashlightOffset;
  MUINT16 FlashlightShutFactor;
...
typedef struct SET_PD_BLOCK_INFO_T {
  ...
  MUINT32 i4BlockNumY;
  MUINT32 i4LeFirst;
  MUINT32 i4Crop[SENSOR_SCENARIO_ID_MAX][2];
  MUINT32 i4VolumeX;
  MUINT32 i4VolumeY;
  MUINT32 i4FullRawW;
  MUINT32 i4FullRawH;
...
} IMGSENSOR_HDR_SUPPORT_TYPE_ENUM;
...
#define MAX_SENSOR_CAL_SIZE_KERNEL (1024)
...
typedef struct {
  MUINT32 SensorId;
  SENSOR_SCENARIO_ID_ENUM ScenarioId;
  ACDK_SENSOR_INFO_STRUCT * pInfo;
  ACDK_SENSOR_CONFIG_STRUCT * pConfig;
} IMGSENSOR_GET_CONFIG_INFO_STRUCT;
typedef struct {
  SENSOR_SCENARIO_ID_ENUM ScenarioId[KDIMGSENSOR_MAX_INVOKE_DRIVERS];
...
typedef struct {
  ...
  MUINT32 u4InClk;
  MUINT32 u4OutClk;
  MUINT32 u4TotalPixelPerLine;
  MUINT32 u4TotalLinesPerFrame;
  MUINT32 u4ActualWidth;
  MUINT32 u4ActualHeight;
  MUINT32 u4Width;
  MUINT32 u4Height;
  MUINT32 u4FrameTimeInus;
  MUINT32 u4MinFrameTimeInus;
  MUINT32 u4LineTimeInus;
  MUINT32 u4FinePixCntPerus;
  MUINT32 u4MinFineTimeInus;
  MUINT32 u4MaxFineTimeInus;
  MUINT32 u4XStart;
  MUINT32 u4XEnd;
  MUINT32 u4YStart;
  MUINT32 u4YEnd;
...
typedef struct {
  unsigned char clklevelcnt;
  unsigned int clklevel[ISP_CLK_LEVEL_CNT];
} IMAGESENSOR_GET_SUPPORTED_ISP_CLK;
...
typedef struct {
  ...
  MUINT32(* SensorControl) (SENSOR_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT * pImageWindow, MSDK_SENSOR_CONFIG_STRUCT * pSensorConfigData);
...
typedef struct {
  ...
  MUINT32(* SensorGetResolution) (MSDK_SENSOR_RESOLUTION_INFO_STRUCT * pSensorResolution[2]);
  MUINT32(* SensorFeatureControl) (CAMERA_DUAL_CAMERA_SENSOR_ENUM InvokeCamera, MSDK_SENSOR_FEATURE_ENUM FeatureId, MUINT8 * pFeaturePara, MUINT32 * pFeatureParaLen);
  MUINT32(* SensorControl) (CAMERA_DUAL_CAMERA_SENSOR_ENUM InvokeCamera, SENSOR_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT * pImageWindow, MSDK_SENSOR_CONFIG_STRUCT * pSensorConfigData);
  MUINT32(* SensorClose) (void);
} MULTI_SENSOR_FUNCTION_STRUCT2, * PMULTI_SENSOR_FUNCTION_STRUCT2;
typedef struct {
  MUINT32(* SensorOpen) (void);
  MUINT32(* SensorGetInfo) (SENSOR_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_INFO_STRUCT * pSensorInfo, MSDK_SENSOR_CONFIG_STRUCT * pSensorConfigData);
...
typedef struct {
  MUINT16 AeRefLV05Shutter;
  MUINT16 AeRefLV13Shutter;
  MUINT16 AeRefLV05Gain;
  MUINT16 AeRefLV13Gain;
} SENSOR_AE_REF_STRUCT, * PSENSOR_AE_REF_STRUCT;
typedef struct {
  MUINT16 AwbRefD65Rgain;
  MUINT16 AwbRefD65Bgain;
  MUINT16 AwbRefCWFRgain;
  MUINT16 AwbRefCWFBgain;
} SENSOR_AWB_GAIN_REF_STRUCT, * PSENSOR_AWB_GAIN_REF_STRUCT;
typedef struct {
  SENSOR_AE_REF_STRUCT SensorAERef;
  SENSOR_AWB_GAIN_REF_STRUCT SensorAwbGainRef;
  MUINT32 SensorLV05LV13EVRef;
} SENSOR_AE_AWB_REF_STRUCT, * PSENSOR_AE_AWB_REF_STRUCT;
typedef struct {
  MUINT16 AeCurShutter;
  MUINT16 AeCurGain;
...
typedef struct {
  SENSOR_AE_CUR_STRUCT SensorAECur;
  SENSOR_AWB_GAIN_CUR_STRUCT SensorAwbGainCur;
...
struct IMGSENSOR_AE_FRM_MODE{
  MUINT32 reserved:16;
  MUINT32 frame_mode_1:4;
  MUINT32 frame_mode_2:4;
